...
// @ts-expect-error - adm-zip does not have type declarations installed
...
export async function extractTextFromFile(filePath: string, mimeType?: string) {
  ...
  try {
    ...
    if (
      ...
      const slides = (entries as any[]).filter(
        (e: any) =>
          ...
          for (const k of Object.keys(node)) {
            if (k === "a:t") {
              if (Array.isArray(node[k]))
                node[k].forEach((v: any) => tMatches.push(String(v)));
    ...
    if (
      ...
      /\.(png|jpe?g|gif|bmp|tiff|webp)$/i.test(lower)
    ) {
      try {
