...
export default async function AnalyticsPage() {
  ...
  try {
    ...
    const a = await anyDb.chatSession?.findFirst({
      where: userId ? { userId } : {},
  ...
  try {
    const anyDb = db as any;
    const a = await anyDb.flashcardSet?.findFirst({
      where: userId ? { userId } : {},
      orderBy: { createdAt: "asc" },
      select: { createdAt: true },
    });
  ...
  try {
    const anyDb = db as any;
    const a = await anyDb.quizSet?.findFirst({
      where: userId ? { userId } : {},
      orderBy: { createdAt: "asc" },
      select: { createdAt: true },
    });
  ...
  startDate.setHours(0, 0, 0, 0); // default: 14 days
  if (earliestDates.length > 0) {
    ...
    maxWindowStart.setDate(today.getDate() - 179);
  ...
  const sessions = await (async () => {
    try {
      const anyDb = db as any;
      return (
        (await anyDb.chatSession?.findMany({
          where: userId ? { userId } : {},
  ...
  const studyTimeByDay = days.map((day) => {
    ...
    const minutes = sessions.reduce((acc: number, s: any) => {
      const durMs = Math.max(
        0,
        (s.lastActiveAt?.getTime?.() ?? s.createdAt.getTime()) -
          s.createdAt.getTime(),
      );
      const midpoint = new Date(s.createdAt);
      midpoint.setHours(0, 0, 0, 0);
      if (s.createdAt >= day && s.createdAt < next)
        return acc + Math.round(durMs / 60000);
      return acc;
    }, 0);
  ...
  const sets = await (async () => {
    try {
      const anyDb = db as any;
      return (
        (await anyDb.flashcardSet?.findMany({
          where: userId ? { userId } : {},
  ...
  const flashcardsByDay = days.map((day) => {
    ...
    const count = sets.reduce(
      (acc: number, s: any) =>
        s.createdAt >= day && s.createdAt < next ? acc + s.count : acc,
      0,
    );
  ...
  const quizzesByDay = days.map((day) => {
    const next = new Date(day);
    next.setDate(day.getDate() + 1);
    const count = quizSets.reduce(
      (acc: number, q: any) =>
        q.createdAt >= day && q.createdAt < next ? acc + 1 : acc,
      0,
    );
    return { day: formatDay(day), count };
  });
  ...
  const scores = await (async () => {
    try {
      const anyDb = db as any;
      return (
        (await anyDb.quizScore?.findMany({
          where: userId ? { userId } : {},
          orderBy: { createdAt: "asc" },
  ...
  const nextEvent = await (async () => {
    try {
      ...
      const now = new Date();
      return (
        (await anyDb.calendarEvent?.findFirst({
          where: userId
            ? { userId, start: { gte: now } }
            : { start: { gte: now } },
          orderBy: { start: "asc" },
  ...
  const scoresSeries = scores.map((s: any) => ({
  ...
  const avgScore = scores.length
    ? Math.round(
        scores.reduce((a: number, s: any) => a + s.percentage, 0) /
          scores.length,
      )
    : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <AnalyticsClient
        ...
        nextEvent={
          nextEvent
            ? { title: nextEvent.title, start: nextEvent.start.toISOString() }
            : null
        }
        totals={{
          minutes: totalStudyMin,
          cards: totalCards,
