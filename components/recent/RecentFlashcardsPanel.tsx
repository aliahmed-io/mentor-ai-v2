...
export function RecentFlashcardsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`${origin}/api/flashcards/recent?limit=25`, {
        cache: "no-store",
      });
      if (!r.ok) {
        setSets([]);
      ...
      console.error("Failed to fetch recent flashcards:", error);
      setSets([]);
    } finally {
      setLoading(false);
    }
  ...
  const rename = async (id: string, currentTitle: string | null) => {
    ...
    try {
      const origin = window.location.origin;
      await fetch(`${origin}/api/flashcards/${id}`, {
        method: "PATCH",
  ...
  const del = async (id: string) => {
    ...
    try {
      const origin = window.location.origin;
      await fetch(`${origin}/api/flashcards/${id}`, { method: "DELETE" });
      await load();
    } catch (error) {
          ...
          <SheetContent side="right" className="w-[420px] sm:max-w-md">
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <Button
                variant="destructive"
                onClick={async () => {
                  if (selectedId) await del(selectedId);
                  setDeleteOpen(false);
                }}
              >
                Delete
