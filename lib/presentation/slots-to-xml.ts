...
export function generatedSlideToXml(
...
function slotToXml(slot: AnySlotContent): string {
  switch (slot.type) {
    ...
    case "chart":
      return (
        `<CHART charttype="${slot.chartType || "bar"}">\n` +
        slot.data
          .map(
            (d) =>
    ...
    case "compare":
      return (
        ...
        slot.sides
          .map(
            (side) =>
              `    <DIV><H3>${escapeXml(side.title)}</H3>` +
              side.items.map((i) => `<LI>${escapeXml(i.text)}</LI>`).join("") +
              `</DIV>`,
          )
          .join("\n") +
        `\n  </COMPARE>`
    ...
    case "pros-cons": {
      ...
      if (pros)
        pcXml +=
          `  <PROS><H3>${escapeXml(pros.title)}</H3>` +
          pros.items.map((i) => `<LI>${escapeXml(i.text)}</LI>`).join("") +
          `</PROS>\n`;
      if (cons)
        pcXml +=
          `  <CONS><H3>${escapeXml(cons.title)}</H3>` +
          cons.items.map((i) => `<LI>${escapeXml(i.text)}</LI>`).join("") +
          `</CONS>\n`;
      pcXml += `</PROS-CONS>`;
      return pcXml;
    }
